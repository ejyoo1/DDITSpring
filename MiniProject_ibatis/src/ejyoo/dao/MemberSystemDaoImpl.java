...
public class MemberSystemDaoImpl implements IMemberSystemDao{
	...
	@Override
	public MemberDTO selectMember(SqlMapClient smc, String userId) throws SQLException {
		try {
			MemberDTO member = (MemberDTO) smc.queryForObject("member.selectMember", userId);
			return member;
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
